...
class ViserViewer(BaseViewer):
  ...
  def setup(self) -> None:
    ...
    with tabs.add_tab("Controls", icon=viser.Icon.SETTINGS):
      ...
      with self._server.gui.add_folder("Display Settings"):
        self._cb_collision = self._server.gui.add_checkbox(
          "Collision geom", initial_value=False
        )
        self._cb_visual = self._server.gui.add_checkbox("Visual geom", initial_value=True)
  ...
  def sync_env_to_viewer(self) -> None:
    ...
    def update_mujoco() -> None:
      # Ensure handles exist if checkboxes are enabled.
      if self._cb_visual.value:
        self._ensure_visual_handles_exist()
      if self._cb_collision.value:
      ...
      with self._server.atomic():
        ...
        for handles_dict, checkbox in [
          (self._mesh_visual_handles, self._cb_visual),
          (self._mesh_collision_handles, self._cb_collision),
        ]:
          ...
          for body_id, handle in handles_dict.items():
            ...
            # Skip pose update if not visible.
