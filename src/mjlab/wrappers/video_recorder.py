...
class VideoRecorder(ManagerBasedRlEnv):
    ...
    def unwrapped(self) -> ManagerBasedRlEnv:
        ...
        # Unwrap through any VideoRecorder wrappers.
        env = self._wrapped_env
        while isinstance(env, VideoRecorder):
            env = env._wrapped_env
        # If the inner env has an unwrapped property, use it.
        if hasattr(env, "unwrapped"):
            return env.unwrapped  # type: ignore
    ...
    def step(self, action: torch.Tensor) -> Any:
        ...
        if self.step_trigger is not None and self.step_trigger(self.step_count):
            should_record = True
        elif self.episode_trigger is not None and self.episode_trigger(
            self.episode_count
        ):
            should_record = True
    ...
    def _finish_recording(self) -> None:
        ...
        if not self.current_video_frames:
            if not self.disable_logger:
                print(f"[WARN] No frames recorded, skipping video save.")
            self.is_recording = False
            return
    ...
    def _save_video(self) -> None:
        """Save recorded frames to an MP4 file using moviepy."""
        if not self.current_video_frames:
            return

        from moviepy import ImageSequenceClip

        frames = self.current_video_frames
        ...
        try:
            ...
            clip.write_videofile(
                str(self.current_video_path), verbose=False, logger=None
            )
        except Exception as e:
            raise RuntimeError(
                f"Failed to save video to {self.current_video_path}: {e}"
            )
