...
def unitree_go2_rough_env_cfg(
  ...
  self_collision_cfg = ContactSensorCfg(
    name="self_collision",
    primary=ContactMatch(mode="subtree", pattern=body_name, entity="robot"),
    secondary=ContactMatch(mode="subtree", pattern=body_name, entity="robot"),
    fields=("found", "force"),
    reduce="none",
    num_slots=1,
    history_length=4,
  )
  thigh_geom_names = tuple(f"{leg}_thigh_collision1" for leg in foot_names)
  ...
  base_ground_cfg = ContactSensorCfg(
    ...
    primary=ContactMatch(
      mode="geom",
      entity="robot",
      pattern=("base_collision1", "base_collision2", "base_collision3"),
    ),
    secondary=ContactMatch(mode="body", pattern="terrain"),
    fields=("found", "force"),
    reduce="none",
  ...
  cfg.events["base_com"].params["asset_cfg"].body_names = (body_name,)
  ...
  cfg.rewards["upright"].params["asset_cfg"].body_names = (body_name,)
  cfg.rewards["upright"].params["terrain_sensor_names"] = ("terrain_scan",)
  cfg.rewards["body_ang_vel"].params["asset_cfg"].body_names = (body_name,)
...
def unitree_go2_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  for key in ("self_collisions", "shank_collision", "base_collision"):
