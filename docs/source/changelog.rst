...
- Added ``mdp.projected_gravity_from_sensor``, an observation that derives
  ...
  sensor; the velocity tasks use this observation and randomize the IMU site
  position/orientation via ``dr.site_pos`` / ``dr.site_quat``, and the G1
  tracking config adds the same randomization.
