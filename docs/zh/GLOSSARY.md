...
## 六、rst 行内标记与 CJK 的边界规则(重要)

docutils 要求粗体 `**x**`、斜体 `*x*`、行内代码 ``` ``x`` ``` 与角色
`:role:`x`` 的标记边界满足:

- **结束标记之后**不能紧跟汉字或全角括号(全角句号、逗号可以)
- **开始标记之前**不能紧邻汉字

违反时整个标记不渲染,星号/反引号字面残留到页面上。

**修复规则**:标记与汉字/全角括号之间插入一个半角空格;
行内代码 `` `x` `` 不允许跨行断开;能用措辞避免紧邻的优先调整措辞。
修改后必须运行 `uv run --group docs python docs/check_cjk_inline.py`
复检清零。

## 七、图片引用规则

中文树与英文树共用 `docs/source/_static` 下的图片资源,但 rst 的
figure/image 引用是相对源文件解析的。中文 rst 引用图片时必须写从
`docs/zh/<该文件位置>` 出发指向 `docs/source/_static` 的相对路径
(逐级 `../` 前缀),保证 sphinx 构建期能读到文件并复制进 `_images`,
否则 href 会被重写为源目录相对路径导致线上 404。
