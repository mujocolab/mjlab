...
class MjxEnv(Generic[TaskT]):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        data = step(
            ...
            before_substep_fn=partial(
                self.task.before_substep, action=action, state=state
            ),
            after_substep_fn=partial(
                self.task.after_substep, action=action, state=state
            ),
        )
        ...
        data = self.task.after_step(data=data, state=state)
