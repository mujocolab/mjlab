...
class RslRlVecEnvWrapper(VecEnv, Generic[ConfigT, TaskT]):
    ...
    def get_observations(self):
        ...
        if self.asymmetric_obs:
            ...
            critic_obs = _jax_to_torch(
                self.state.obs["privileged_state"], device=self.device
            )
    ...
    def reset(self):
        ...
        self.reset_time_outs = torch.zeros_like(
            self.reset_terminated, device=self.device
        )
